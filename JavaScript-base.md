...
20. 作用域：作用域就是变量与函数的可访问范围，变量的作用域有全局作用域和局部作用域两种。
	...
	3. 全局属性：undefined，NaN，Infinity；全局函数：parseInt()；全局对象：math，json；构造函数：Date()等。
...
55. MVC框架（模型--视图--控制器）：M是业务模型，V是用户界面，C是控制器，该框架目的是将模型和视图代码分离，从而使同一程序可以使用不同的表现形式。C存在的目的则是确保M和V的同步，一旦M改变，V应该同步更新。Model（模型）是应用程序中用于处理应用程序数据逻辑的部分。通常模型对象负责在数据库中存取数据。View（视图）是应用程序中处理数据显示的部分。通常视图是依据模型数据创建的。Controller（控制器）是应用程序中处理用户交互的部分。通常控制器负责从视图读取数据，控制用户输入，并向模型发送数据。优点：1.MVC相互独立，视图层与业务层相分离，更改代码而互不影响，可维护性高。2.使用各种不同样式的视图来访问同一个服务器端的代码，因为多个视图能共享一个模型，比如，用户可以通过电脑也可通过手机来订购某样产品。3.分工明确，精力更集中在各表现形式上，java在业务逻辑，界面在html上。缺点：1.原理复杂，需要精心规划，不适合中小型项目。

54. 原生js和其他js的区别：以往是先写一个页面，再操作dom元素，但是angularjs等是要设计架构，展示视图，并且数据绑定，自动更新视图
...
59. 前端数据可视化插件:Highcharts、Echarts和D3：可视化是利用计算机图形学和图像处理技术，将数据转换成图形或图像在屏幕上显示出来。

	1. Highcharts 兼容 IE6 及以上的所有主流浏览器，完美支持移动端缩放、手势操作。Highcharts 基于SVG，方便自己定制，但图表类型有限。
...
62. React 特点
	...
	3. 1.声明式设计 −React采用声明范式，可以轻松描述应用。
	4. 高效 −React通过对DOM的模拟，最大限度地减少与DOM的交互。
...
63. js如何定义类
    Javascipt语法不支持"类"（class），导致传统的面向对象编程方法无法直接使用。但是可以用Javascript模拟"类"。以下总结了Javascript定义"类"的三种方法。类可以看作是对象类型的子类型，除了数组和函数类外，日期，正则，错误也是常用的类。
    1. 构造函数法，函数用来初始化一个新建对象（new一个对象），而每个函数定义了一个类对象。
        ```
        function Cat() {
            this.name = "大毛";
        }
...
64.  String、String.valueOf、toString 它们三者的区别总结？
